...
## Phase 6: Stage-1 Minimal Joint Objective

Implemented:

- Added `python/pinhole_calib/joint_intrinsics.py`.
- Added:
  - `JointFramePriors`
...
- The optimizer now combines:
  - normalized geometry reprojection residual
  - corrected pointmap-to-geometry residual
  - corrected depth-derived-pointmap-to-geometry residual
  - corrected normal/pointmap consistency residual
  - focal anchoring around the geometry-only initialization
- Added synthetic coverage in:
  - `python/tests/test_joint_intrinsics.py`

Important debugging path:

- First attempt used pointmap intrinsic self-consistency and failed to improve over geometry-only.
- That was rejected because it rewarded self-consistency more than correction.
- The objective was then rewritten so the prior terms measure corrected-prior disagreement with geometry.
- A second failure showed geometry residuals were overweighted by pixel units and residual count.
- That was corrected by normalizing geometry residuals.
- A third failure showed focal drift; this was stabilized with a focal-anchor prior.
- On real smoke runs, the joint objective was initially too aggressive and pulled `K` badly.
- `stage_ab` now uses a conservative weak-prior configuration plus per-channel gating for the joint probe.
...
- Full unit tests passed:
  - `24 tests`
  - `OK`
- Synthetic acceptance now passes:
  - joint optimizer improves over geometry-only on the controlled biased-geometry / clean-prior test
- Courtroom smoke:
  - `python/output/12/courtroom_stage_ab_smoke.json`
  - separable error norm: `~0.0263`
  - joint-probe error norm: `~0.0294`
  - joint probe does **not** beat separable, but no longer catastrophically destabilizes the estimate

Interpretation:

- This stage now passes as a **conservative minimal joint objective**.
- It is good enough to exist as a real stage-1 joint solver and to support the next stage.
- It is **not** good enough to replace the separable estimate as the default `K`.
- The honest conclusion is:
  - the joint core now exists,
  - it can help on controlled data,
  - and on the current smoke it behaves cautiously rather than destructively.
